...
export default async function AboutPage() {
  ...
  const intro = content['about.intro'] ?? "Jovie Joy started in 2023 when Mel and Ross couldn't find colouring pages they actually liked for their daughter Jovie (hi, namesake!). So they drew their own. Then their friends asked for copies. Then the friends' friends did. So they opened a little shop and here we are.";
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 20, margin: '50px 0' }}>
          {stats.map((s, i) => (
            <div key={i} className="card" style={{ background: s.color, color: s.light ? 'var(--ink)' : 'var(--cream)', textAlign: 'center' }}>
              <div className="heading" style={{ fontSize: 56, lineHeight: 1 }}>{s.n}</div>
              <div style={{ fontSize: 14 }}>{s.l}</div>
            </div>
          ))}
        </div>

        <h2 className="heading" style={{ fontSize: 44, marginBottom: 18 }}>Our promise</h2>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
          {promises.map((p, i) => (
            <div key={i} className="card">
              <div className="heading" style={{ fontSize: 22, marginBottom: 4 }}>{p.t}</div>
