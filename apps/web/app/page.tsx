        ...
        <div className="page" style={{ position: 'relative', zIndex: 1 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: 60, alignItems: 'center' }}>
            <div className="fade-up">
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '6px 14px', background: 'var(--mint)', border: '2.5px solid var(--ink)', borderRadius: 999, fontSize: 13, marginBottom: 20 }}>
                <span style={{ width: 8, height: 8, background: 'var(--ink)', borderRadius: '50%' }} />
                {content['home.announcement'] ?? 'Lower prices, bigger smiles'}
              </div>
              ...
              <div style={{ marginTop: 40, display: 'flex', gap: 24, alignItems: 'center' }}>
                ...
                <div>
                  <div style={{ fontWeight: 800, fontSize: 17 }}>★★★★★ 4.9</div>
      ...
      <section style={{ padding: '60px 0', background: 'var(--cream-2)', position: 'relative' }}>
        ...
        <div className="page" style={{ padding: '40px 32px' }}>
          <div style={{ textAlign: 'center', marginBottom: 50 }}>
            ...
            <h2 className="heading" style={{ fontSize: 56, margin: 0 }}>Three tiny steps</h2>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 28 }}>
            {steps.map(s => (
              <div key={s.n} className="card" style={{ background: s.color, transform: `rotate(${s.rot}deg)` }}>
                <div className="heading" style={{ fontSize: 64, color: 'var(--paper)', WebkitTextStroke: '2.5px var(--ink)', lineHeight: 1, marginBottom: 10 }}>{s.n}</div>
                <div className="heading" style={{ fontSize: 28, marginBottom: 8 }}>{s.t}</div>
        ...
        <div className="page">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 40 }}>
            <div>
              ...
              <h2 className="heading" style={{ fontSize: 54, margin: 0 }}>Fan favourites</h2>
            </div>
      ...
      <section style={{ padding: '80px 0', background: 'var(--ink)', color: 'var(--cream)' }}>
        <div className="page">
          <div style={{ textAlign: 'center', marginBottom: 50 }}>
            ...
            <h2 className="heading" style={{ fontSize: 56, margin: 0 }}>Parents who get it</h2>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 20 }}>
            {testimonials.map((q, i) => (
              <div key={i} style={{ background: q.color, color: q.w ? 'var(--cream)' : 'var(--ink)', border: '2.5px solid var(--cream)', borderRadius: 20, padding: 22, transform: `rotate(${[-2, 1, -1, 2][i]}deg)` }}>
                <div style={{ fontSize: 32, lineHeight: 0.5, marginBottom: 6}}>&ldquo;</div>
                <div style={{ fontSize: 16, marginBottom: 16, lineHeight: 1.4 }}>{q.q}</div>
                <div style={{ fontWeight: 800, fontSize: 13 }}>— {q.a}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="page">
          <div style={{
            ...
            <div style={{ position: 'relative', zIndex: 1 }}>
              ...
              <h2 className="heading" style={{ fontSize: 58, margin: '0 0 16px' }}>
