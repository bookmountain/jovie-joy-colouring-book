...
export function ChooseOptionsModal() {
  const { state, dispatch } = useSite();

  if (state.activeModal !== "choose-options" && state.activeModal !== "edit-option") {
    return null;
  }
    ...
    <div className="fixed inset-0 z-50 grid place-items-center bg-cocoa-ink/35 p-4">
      <section className="w-full max-w-md rounded-coco bg-cocoa-cream p-6 shadow-drawer">
        <div className="mb-5 flex items-center justify-between">
          <h2 className="text-2xl font-extrabold">Choose options</h2>
          <button
        ...
        <p className="text-sm leading-6 text-cocoa-text">
          Product option selection is simulated locally in this clone.
        </p>
      </section>
    </div>
  );
